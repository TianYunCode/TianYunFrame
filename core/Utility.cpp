...
namespace WTY
{
    ...
    // bdelspace：是否删除拆分后的字段内容前后的空格，true-删除；false-不删除，默认不删除。
    ...
    string& timetostr(const time_t ttime, string& strtime, const string& fmt)
    {
        ...
        {
            strtime = sformat("%04u-%02u-%02u %02u:%02u:%02u", sttm.tm_year, sttm.tm_mon, sttm.tm_mday, sttm.tm_hour, sttm.tm_min, sttm.tm_sec);
            return strtime;
        }

        if (fmt == "yyyy-mm-dd hh24:mi")
        {
            strtime = sformat("%04u-%02u-%02u %02u:%02u", sttm.tm_year, sttm.tm_mon, sttm.tm_mday, sttm.tm_hour, sttm.tm_min);
            return strtime;
        }
    ...
    time_t strtotime(const string& strtime)
    {
        ...
        // 如果时间格式不是yyyymmddhh24miss，说明时间格式不正确。
        if (strtmp.length() != 14)
        {
            return -1;
        }
    ...
    bool ctcpclient::connect(const string& ip, const int port)
    {
        ...
        // 如果send到一个disconnected socket上，内核就会发出SIGPIPE信号。这个信号的默认处理方法是终止进程，大多数时候这都不是我们期望的。我们重新定义这个信号的处理方法，大多数情况是直接屏蔽它。
    ...
    bool ctcpserver::initserver(const unsigned int port, const int backlog)
    {
        ...
        // 忽略SIGPIPE信号，防止程序异常退出。
        // 如果往已关闭的socket继续写数据，会产生SIGPIPE信号，它的默认行为是终止程序，所以要忽略它。
        signal(SIGPIPE, SIG_IGN);
