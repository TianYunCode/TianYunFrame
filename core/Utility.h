...
public:
    ...
    // bdelspace：拆分后是否删除字段内容前后的空格，true-删除；false-不删除，默认不删除。
    ...
    bool getvalue(const int ii, char *value, const int ilen = 0) const;   // C风格字符串，ilen默认值为0-全部长度。
...
// 解析xml格式字符串的函数族。
// xml格式的字符串的内容如下：
// <filename>/tmp/_public.h</filename><mtime>2024-01-01 12:20:35</mtime><size>18348</size>
// <filename>/tmp/_public.cpp</filename><mtime>2024-01-01 10:10:15</mtime><size>50945</size>
// xmlbuffer：待解析的xml格式字符串。
// fieldname：字段的标签名。
// value：传入变量的地址，用于存放字段内容，支持bool、int、insigned int、long、unsigned long、double和char[]。
// 注意：当value参数的数据类型为char []时，必须保证value数组的内存足够，否则可能发生内存溢出的问题，也可以用ilen参数限定获取字段内容的长度，ilen的默认值为0，表示不限长度。
...
/*
      取操作系统的时间（用字符串表示）。
      strtime：用于存放获取到的时间。
      timetvl：时间的偏移量，单位：秒，0是默认值，表示当前时间，30表示当前时间30秒之后的时间点，-30表示当前时间30秒之前的时间点。
      fmt：输出时间的格式，fmt每部分的含义：yyyy-年份；mm-月份；dd-日期；hh24-小时；mi-分钟；ss-秒，
      默认是"yyyy-mm-dd hh24:mi:ss"，目前支持以下格式：
...
// bisfilename：指定pathorfilename的类型，true-pathorfilename是文件名，否则是目录名，默认值为true。
...
// fmt：设置时间的输出格式，与ltime()函数相同，但默认是"yyyymmddhh24miss"。
...
private:
    ...
    string m_fmt;              // 文件时间格式，默认"yyyymmddhh24miss"。

    cdir(const cdir &) = delete;            // 禁用拷贝构造函数。
    cdir &operator=(const cdir &) = delete; // 禁用赋值函数。
public:
    // /DataSharingPlatform/public/_public.h
    string m_dirname;   // 目录名，例如：/DataSharingPlatform/public
    string m_filename;  // 文件名，不包括目录名，例如：_public.h
    string m_ffilename; // 绝对路径的文件，例如：/DataSharingPlatform/public/_public.h
    ...
    // 设置文件时间的格式，支持"yyyy-mm-dd hh24:mi:ss"和"yyyymmddhh24miss"两种，默认是后者。
    void setfmt(const string &fmt);

    // 打开目录，获取目录中文件的列表，存放在m_filelist容器中。
    // dirname，目录名，采用绝对路径，如/tmp/root。
    // rules，文件名的匹配规则，不匹配的文件将被忽略。
    // maxfiles，本次获取文件的最大数量，默认值为10000个，如果文件太多，可能消耗太多的内存。
    // bandchild，是否打开各级子目录，默认值为false-不打开子目录。
    // bsort，是否按文件名排序，默认值为false-不排序。
...
public:
    ...
    // 以行的方式读取文本文件，endbz指定行的结尾标志，默认为空，没有结尾标志。
...
public:
    ...
    // openmode：日志文件的打开模式，默认值是ios::app。
    // bbackup：是否自动切换（备份），true-切换，false-不切换，在多进程的服务程序中，如果多个进程共用一个日志文件，bbackup必须为false。
    // benbuffer：是否启用文件缓冲机制，true-启用，false-不启用，如果启用缓冲区，那么写进日志文件中的内容不会立即写入文件，默认是不启用。
...
private:
    ...
    // 备份后的文件会在日志文件名后加上日期时间，如/tmp/log/filetodb.log.20240101123025。
...
// 忽略关闭全部的信号、关闭全部的IO，默认只忽略信号，不关IO。
