...
// 设置按钮样式
QString FlatUI::setPushButtonQss(QPushButton *btn, int radius, int fontSize,
...
{
    QStringList list;
    list.append(QString("QPushButton{border-style:none;border-radius:%1px;font-size:%2px;color:%3;background:%4;}")
                .arg(radius).arg(fontSize).arg(normalTextColor).arg(normalColor));
...
// 设置文本框样式
QString FlatUI::setLineEditQss(QLineEdit *txt, int radius, int fontSize, const QString &fontColor, int borderWidth,
                               const QString &normalColor,
                               const QString &focusColor)
{
    QStringList list;
    list.append(QString("QLineEdit{border-style:none;padding:3px;border-radius:%1px;font-size:%2px;color:%3;border:%4px solid %5;}")
                .arg(radius).arg(fontSize).arg(fontColor).arg(borderWidth).arg(normalColor));
