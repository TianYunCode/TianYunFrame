...
namespace rest_rpc
{
	namespace util
	{
		...
		struct is_invocable : std::is_constructible<std::function<void(std::remove_reference_t<Args>...)>, std::reference_wrapper<typename std::remove_reference<F>::type>> {};
		...
		{
			requires std::is_array_v<std::remove_reference_t<T>>&& std::same_as<std::remove_extent_t<std::remove_cvref_t<T>>, char>;
		};

		template <typename T>
		concept CharArray = std::is_array_v<std::remove_reference_t<T>> && std::same_as<std::remove_extent_t<std::remove_cvref_t<T>>, char>;

		template <typename T, typename...>
		inline constexpr bool is_basic_v = std::is_fundamental_v<T> || string<T> || CharArray<T> || CharArrayRef<T>;
	} // namespace util

	template <typename Function>
	using function_return_type_t = typename util::function_traits<std::remove_cvref_t<Function>>::return_type;

	template <typename Function>
	using function_parameters_t = typename util::function_traits<std::remove_cvref_t<Function>>::parameters_type;
} // namespace rest_rpc