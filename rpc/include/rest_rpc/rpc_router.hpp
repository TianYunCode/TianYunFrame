...
{
	template <typename T>
	constexpr inline bool is_void_v = std::is_same_v<T, void> || std::is_same_v<T, asio::awaitable<void>>;
	...
	struct rpc_result
	{
		...
		bool empty() const
		{
			return result.empty() && view.empty();
		}

		size_t size() const
		{
			return result.empty() ? view.size() : result.size();
		}

		std::string_view data() const
		{
			return result.empty() ? view : result;
		}
	};
	...
	public:
		...
		void register_handler(std::string_view name, const Function& f, Self* self = nullptr)
		{
		...
		asio::awaitable<rpc_result> route(uint32_t key, std::string_view data)
		{
			...
			{
				route_result.result = std::string("exception occur when call").append(ex.what());
				route_result.ec = rpc_errc::function_exception;
			}
			catch (...)
			{
				route_result.result = std::string("unknown exception occur when call ").append(get_name_by_key(key));
	...
	private:
		template <typename Function, typename Self = void>
		void register_handler_impl(uint32_t key, std::string_view name, const Function& f, Self* self = nullptr)
		{
		...
		{
			this->map_invokers_[key] = [this, f, self](std::string_view str, rpc_result& ret) -> asio::awaitable<void>
				{
					...
					else
					{
						...
						if constexpr (std::tuple_size_v<args_tuple> == 1 && util::is_basic_v<first_t>)
						{
		...
		asio::awaitable<void> handle_zero_arg(const F& f, rpc_result& ret, Self* self)
		{
		...
		asio::awaitable<void> handle_one_arg(std::string_view str, const F& f, rpc_result& ret, Self* self)
		{
			if constexpr (is_void_v<R>)
			{
				if constexpr (std::is_void_v<Self>)
			...
			else
			{
				...
				else
				{
					if constexpr (is_awaitable_v<R>)
					{
						ret = rpc_codec::pack_args(co_await(*self.*f)(rpc_codec::unpack<Arg>(str)));
					}
					else
					{
		...
		asio::awaitable<void> handle_more_args(std::string_view str, const F& f, rpc_result& ret, Self* self)
		{
			...
			if constexpr (std::is_void_v<R>)
			{
				...
				else
				{
					if constexpr (is_awaitable_v<R>)
					{
						co_await std::apply([self, &f](auto &&...args) { return (*self.*f)(std::forward<decltype(args)>(args)...); }, tp);
					}
					else
					{
						std::apply([self, &f](auto &&...args) { return (*self.*f)(std::forward<decltype(args)>(args)...); }, tp);
					}
				}
			}
			else
			{
				...
				else
				{
					if constexpr (is_awaitable_v<R>)
					{
						ret = rpc_codec::pack_args(co_await std::apply([self, &f](auto &&...args) { return (*self.*f)(std::forward<decltype(args)>(args)...); }, tp));
					}
					else
					{
						ret = rpc_codec::pack_args(std::apply([self, &f](auto &&...args) { return (*self.*f)(std::forward<decltype(args)>(args)...); }, tp));
					}
				}
			}
			co_return;
		...
		std::unordered_map<uint32_t, std::function<asio::awaitable<void>(std::string_view, rpc_result&)>> map_invokers_;
