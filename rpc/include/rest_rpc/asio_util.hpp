...
{
	template <typename T>
	constexpr inline bool is_awaitable_v = util::is_specialization_v<std::remove_cvref_t<T>, asio::awaitable>;
	...
	inline auto async_start(auto executor, auto&& coro, auto callback)
	{
		...
		{
			asio::co_spawn(executor, std::move(coro), asio::any_completion_handler<void(std::exception_ptr)>
				([cb = std::move(callback)](std::exception_ptr e) { cb(e); }));
		}
		else
		{
			asio::co_spawn(executor, std::move(coro), asio::any_completion_handler<void(std::exception_ptr, R)>
				([cb = std::move(callback)](std::exception_ptr e, R r) { cb(e, std::move(r)); }));
		}
	}
} // namespace rest_rpc