...
namespace rest_rpc
{
	...
	namespace MD5
	{
		struct MD5CE
		{
			...
			static constexpr uint8_t GetPaddedMessageByte(const char* data, const uint32_t n, const uint32_t m, const uint32_t i)
			{
				//    DCHECK_LT(i, m);
				//    DCHECK_LT(n, m);
				//    DCHECK_EQ(m % 64, 0u);
			...
			static constexpr uint32_t GetPaddedMessageWord(const char* data, const uint32_t n, const uint32_t m, const uint32_t i)
			{
				//    DCHECK_EQ(i % 4, 0u);
				//    DCHECK_LT(i, m);
				//    DCHECK_LT(n, m);
			...
			static constexpr uint32_t LeftRotate(const uint32_t value, const uint32_t bits)
			{
				//    DCHECK_LT(bits, 32u);
