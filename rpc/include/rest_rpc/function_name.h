...
namespace rest_rpc
{
	namespace detail
	{
		...
		constexpr auto qualified_name_of_impl() noexcept
		{
			...
			constexpr std::string_view keyword{ "rest_rpc::detail::qualified_name_of_impl<" };
...
#error "Unsupported compiler."
			...
			constexpr auto intermediate = signature.substr(prefix_size + additional_size, signature.size() - prefix_size - additional_size - suffix_size);
			...
			{
				constexpr std::string_view str = result.substr(rpos + anonymous_namespace.size());
...
namespace rest_rpc
{
	...
	constexpr std::string_view
		string_view_array_has(const std::array<std::string_view, N>& array, std::string_view value)
	{
