...
namespace rest_rpc
{
	...
	{
	public:
		void set_connection(std::shared_ptr<rpc_connection> conn)
		{
			conn_ = conn;
		}
		...
		void set_delay(bool r)
		{
			delay_ = r;
		}

		auto get_executor();
		std::shared_ptr<rpc_connection> get_conn()
		{
			return conn_;
		}
	...
	{
	public:
		rpc_connection(tcp_socket socket, uint64_t conn_id, rpc_router& router, bool& cross_ending)
			: socket_(std::move(socket)), conn_id_(conn_id), router_(router), cross_ending_(cross_ending)
		{
		}
		...
		asio::awaitable<void> start()
		{
			...
			while (true)
			{
				...
				size_t size;
				set_last_time();
				std::tie(ec, size) = co_await asio::async_read(socket_, asio::buffer(&header, sizeof(rest_rpc_header)), asio::as_tuple(asio::use_awaitable));
				if (ec)
				{
				...
				{
					set_last_time();
					std::tie(ec, size) = co_await asio::async_read(socket_, asio::buffer(body_), asio::as_tuple(asio::use_awaitable));
					if (ec)
					{
		...
		asio::awaitable<std::error_code> response(const rpc_result& result, uint32_t func_id = 0)
		{
			...
			set_last_time();
			auto [ec, size] = co_await asio::async_write(socket_, buffers, asio::as_tuple(asio::use_awaitable));
			if (ec)
			{
				REST_LOG_WARNING << "write error: " << ec.message();
				close();
	...
	private:
		...
		std::chrono::system_clock::time_point last_rwtime_ = std::chrono::system_clock::now();
	...
	asio::awaitable<std::error_code> rpc_context::response_s(Args &&...args)
	{
		...
		static_assert(std::is_constructible_v<args_tuple, Args...>, "rpc function return type and response arguments are not match");
