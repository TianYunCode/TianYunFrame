...
namespace rest_rpc
{
	namespace detail
	{
		...
		struct has_user_pack<std::void_t<decltype(serialize(std::declval<user_codec::rest_adl_tag>(), std::declval<Args>()...))>, Args...> : std::true_type {};
	...
	struct rpc_codec
	{
		template <typename... Args> inline static auto pack_args(Args &&...args)
		{
			...
			else
			{
				...
				{
					return serialize(user_codec::rest_adl_tag{}, std::forward<Args>(args)...);
				}
				else
				{
	...
	private:
		...
		{
			if constexpr (util::CharArrayRef<Arg> || util::CharArray<Arg> || util::string<Arg>)
			{
