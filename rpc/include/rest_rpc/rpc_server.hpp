...
namespace rest_rpc
{
	...
	{
	public:
		rpc_server(std::string address, size_t num_thread = std::thread::hardware_concurrency())
			: io_context_pool_(num_thread), acceptor_(io_context_pool_.get_io_context()), check_timer_(io_context_pool_.get_io_context())
		{
		...
		rpc_server(std::string host, std::string port, size_t num_thread = std::thread::hardware_concurrency())
			: io_context_pool_(num_thread), acceptor_(io_context_pool_.get_io_context()), host_(std::move(host)),
			port_(std::move(port)), check_timer_(io_context_pool_.get_io_context())
		{
		}
		~rpc_server()
		{
		...
		void set_conn_max_age(std::chrono::steady_clock::duration dur)
		{
			if (dur > std::chrono::steady_clock::duration::zero())
			{
				...
				asio::co_spawn(check_timer_.get_executor(), start_check_timer(), asio::detached);
			}
		}
		...
		{
			return has_stop_.load(std::memory_order_acquire);
		}
		...
		{
			router_.remove_handler(name);
		}

		template <auto func> void remove_handler()
		{
			router_.remove_handler<func>();
		}

		void enable_tcp_no_delay(bool r)
		{
			tcp_no_delay_ = r;
		}

		void enable_cross_ending(bool r)
		{
			cross_ending_ = r;
		}
		...
		asio::awaitable<void> publish(std::string_view topic, T&& t)
		{
			...
			for (auto& [_, conn] : conns)
			{
				if (conn->topic_id() == id)
				{
					co_await conn->response(rpc_codec::pack_args(std::forward<T>(t)), id);
				}
			}
		}
	...
	private:
		...
		asio::awaitable<void> accept()
		{
			...
			while (true)
			{
				...
				auto [ec] = co_await acceptor_.async_accept(socket, asio::as_tuple(asio::use_awaitable));
				...
				auto conn = std::make_shared<rpc_connection>(std::move(socket), conn_id, router_, cross_ending_);
				if (need_check_)
				{
			...
			std::call_once(start_flag_, [this, async]
				{
					...
					{
						auto future = asio::co_spawn(acceptor_.get_executor(), accept(), asio::use_future);
						future.wait();
					}
				});
		...
		asio::awaitable<void> check_timeout()
		{
...

			{
				...
				{
					if (cur_time - co_await it->second->get_last_rwtime() > timeout_duration_)
					{
		...
		asio::awaitable<void> start_check_timer()
		{
			while (true)
			{
				...
				auto [ec] = co_await check_timer_.async_wait(asio::as_tuple(asio::use_awaitable));
				if (ec || stop_timer_)
				{
					co_return;
				}
		...
		std::chrono::steady_clock::duration check_duration_ = std::chrono::seconds(12);
		std::chrono::steady_clock::duration timeout_duration_{ std::chrono::seconds(10) };
